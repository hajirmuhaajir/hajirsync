(function() {
    ...
    function parseLyrics() {
        ...
        document.getElementById('lineCount').textContent = lyrics.filter(function(l) { return l.trim(); }).length + ' lines';
        renderSyncList();
        updateProgress();
    }
    ...
    window.addTimestamp = function() {
        ...
        if (lineEl) { lineEl.classList.remove('no-timestamp'); lineEl.classList.add('has-timestamp'); }
    ...
    window.setMode = function(m) {
        ...
        document.getElementById('modeLineBtn').className =
            'px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ' + (m === 'line' ? 'btn-primary' : 'btn-ghost-glass');
        document.getElementById('modeWordBtn').className =
            'px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ' + (m === 'word' ? 'btn-primary' : 'btn-ghost-glass');
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if (e.ctrlKey && e.key === 'z') { e.preventDefault(); window.undoAction(); return; }
        if (e.ctrlKey && e.key === 'y') { e.preventDefault(); window.redoAction(); return; }
        if (e.ctrlKey && e.key === 'p') { e.preventDefault(); window.togglePlay(); return; }
        if (e.ctrlKey && e.key === 'h') { e.preventDefault(); window.openModal('findReplaceModal'); return; }
        if (tag === 'INPUT' || tag === 'SELECT') return;
        if (e.key === ' ' && tag !== 'TEXTAREA') { e.preventDefault(); window.addTimestamp(); return; }
        if (e.key === 'ArrowLeft') { e.preventDefault(); window.seekRelative(-2); return; }
        if (e.key === 'ArrowRight') { e.preventDefault(); window.seekRelative(2); return; }
        if (e.key === 'Enter' && tag !== 'TEXTAREA') { e.preventDefault(); window.addTimestamp(); return; }
    });
